...
export const refreshTokenController = async (req: Request, res: Response, next: NextFunction) => {
  ...
  if (!refreshToken) {
    return res.status(HTTP_STATUS.UNAUTHORIZED).json({ message: 'Refresh token not found' })
  }
  ...
  if (!isValid) {
    return res.status(HTTP_STATUS.FORBIDDEN).json({ message: 'Invalid refresh token' })
  }

  // 3. Xác thực JWT của refresh token
  jwt.verify(refreshToken, process.env.REFRESH_TOKEN_SECRET as string, (err: any, user: any) => {
    if (err) {
      return res.status(HTTP_STATUS.FORBIDDEN).json({ message: 'Invalid refresh token signature' })
    }

    // 4. Tạo một accessToken MỚI (không tạo refresh token mới ở đây)
    const newRefreshToken = AccountService.createRefreshToken(user.account_id, user.email, user.password)
